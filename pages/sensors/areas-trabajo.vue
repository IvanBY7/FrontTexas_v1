  ...
  <div class="container m-3">
    <div class="contenido">
      <div class="columns is-multiline">
        <div v-for="(dispositivo, index) in lista_dispositivos" :key="index" class="column is-one-quarter">
          <div class="card empresa-card" @click="mostrarDetalles(dispositivo)">
            <div class="card-image">
              <figure class="image is-4by3">
                <img src="@/assets/recursos/dispositivo.png" alt="Imagen de empresa">
              </figure>
              <div class="empresa-name">
                <p>Dispostivo: {{dispositivo.IdDispositivo}}-{{ dispositivo.Modelo }}</p>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mixins: [redirect],
  fetch () {
    this.$store.commit('setTitleStack', ['Areas de Trabajo'])
  },
  data () {
    return {
      horaActual: '',
  ...
  methods: {
    async getDispositivos () {
      try {
        const response = await this.$store.dispatch('modules/sensors/getDispositivos')
        this.lista_dispositivos = response.results
      } catch {
        this.$buefy.snackbar.open({
          message: 'Eror al cargar dispositivos',
          type: 'is-danger'
        })
      }
    },
    DatosSucursal () {
      this.formSucursal.IdSucursal = this.$route.query.IdSucursal
      this.formSucursal.Nombre_Sucursal = this.$route.query.Nombre_Sucursal
      return this.formSucursal.Nombre_Sucursal
    },
    rutaimg () {
    ...
    mostrarDetalles (dispositivo) {
      ...
      this.$router.push({ path: '/sensors/Sensores', query: { IdDispositivo: id } })
    },
    AltaRegion (lista) {
